...
class ATRCalculator(BaseMetricCalculator):
    """
    Alpha/Theta Ratio
    """
    ...
    def compute_metric(self) -> float:
        ...
        theta = self.spectrum.band_power_per_channel("theta")
        if alpha.size == 0 or theta.size == 0:
            return 0.0
...
class ECDF:
    """
    캘리브레이션 값의 [low, high] 안 샘플로 ECDF를 만들고,
    re-index = 1 - idx/n (idx = sorted_vals에서 ≤ v 인 개수, side='right').
    실험 재현용으로 예전과 동일한 정의를 유지한다.
    (idx=0이면 1.0; idx=1이면 최대 1-1/n — n이 크면 0.99999 근처 가능)
    """
...
def create_metric_calculator(
    ...
    if name == "tbr":
        ...
        return ATRCalculator(sample_rate_hz, step_seconds, fft_size, ema_alpha, num_channels)
    else:
